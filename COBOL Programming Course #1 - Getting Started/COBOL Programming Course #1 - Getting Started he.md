...
### יצירת קובץ קונפיגורציה לצוות

בצע את השלבים הבאים:

1. נווט לתוסף Zowe Explorer ובדוק אם כבר יש לך קובץ קונפיגורציה לצוות של zosmf.
...
1. לחץ באמצעות לחצן העכבר הימני על קובץ קונפיגורציית הצוות שברצונך לערוך.
...
5. ודא שאתה מחובר למארח הנכון: 204.90.115.200, והגדר את 'rejectUnauthorized' ל-false עבור חיבור מוצלח.
...
בסעיף זה, למדת את התכונות הבסיסיות של תוסף Zowe Explorer וכיצד ליצור ולעבוד עם פרופיל `zosmf` תואם Zowe.

\newpage

# השימוש בתוסף Z Open Editor
...
- **ניווט בקוד**
     ...
     - **תצוגת Breadcrumb view (Breadcrumb view)**
     - **קפיצה להצהרה / הפניה**
...
&#x200f;IBM Z Open Editor הוא תוסף חינמי עבור Visual Studio Code (VSCode) המספק תמיכה בשפות COBOL, PL/I ו-JCL. יחד עם תמיכה זו בשפות, היא מספקת גם סיוע בתוכן ליישומים הקוראים לממשקי API של CICS, MQ ו-DB2. קוד המקור אפילו לא צריך להיות על z/OS, אלא יכול להיות על המחשב המקומי. למרות שקורס זה מתמקד ב-COBOL כשפת כתיבת הקוד, רבות מהפונקציות שנדון בהן יחולו גם על PL/I ו-JCL.
...
מיקרוסופט תכננה את פרוטוקול שרת השפה (LSP) כדי לשמש תיאור משותף לאופן שבו יש ליישם תכונות כמו השלמה אוטומטית עבור שפה ספציפית. שפות שיש להן שרת LSP מיושם זמינות לעבודה בכל עורך התומך ב-LSP. חברות רבות וקהילת הקוד הפתוח שיתפו פעולה כדי לספק שרתי LSP למגוון שפות שונות.
...
### תצוגת Breadcrumb view

באופן דומה, תצוגת Breadcrumb view בחלק העליון של העורך יכולה להראות היכן שורת הקוד הנוכחית קיימת במבנה קוד המקור של COBOL. בזמן שאתה מנווט בקוד המקור בעורך, הנתיב שיוצב ב- Breadcrumb view יתעדכן אוטומטית כדי לשקף היכן אתה נמצא במבנה התוכנית ומספק לך מנגנון לעבור לרכיב אחר בקוד. שוב, אם תפתח את CBL0001 ב-VSCode ותקפוץ לשורה 50, שורה זו היא הצהרה של השדה USA-STATE בתוך המבנה ACCT-FIELDS, ב-FILE-SECTION של ה-DATA-DIVISION. בחלק העליון של העורך, נתיב Breadcrumb view יציג את המידע המוצג באיור 4.

![](Images/image013.png)

*איור 4. נתיב Breadcrumb view ב-VSCode*

לחיצה על כל אחד מהפריטים בשביל Breadcrumb view תדגיש את אותו רכיב בקוד בעורך, ותראה לך במהירות את מיקום הרכיב הזה בקוד. זה גם יציג תצוגה של הקוד בחלון קופץ, המוצג באיור 5, בדומה לתצוגת המיתאר שנדונה קודם לכן.

![](Images/image016.png)

*איור 5. תצוגה קופצת של קוד באמצעות בחירת Breadcrumb view*
...
מיד תבחין שגם הצהרת המשתנה וגם ההפניה בשורה 88 עודכנו לערך החדש. כפי שצוין קודם לכן, אותו תהליך עובד גם עבור שמות פסקאות. לדוגמה, אתה מוזמן לארגן מחדש את שם הפסקה READ-RECORD ל-READ-NEW-RECORD.
...
# חבילת הההרחבות בקוד פתוח Code4z
...
- **תמיכה ב-Copybook**
- **השלמה אוטומטית (Autocomplete)**דד
- **סיכום**

## מהו Code4z?

חבילת Code4z מכילה הרחבות המספקות תמיכה בשפות COBOL ו-High-Level Assembler, דיבאג לתוכניות COBOL, וכן כלים המאפשרים למפתחים לגשת לdata sets של mainframe ולמאגרי קוד של CA Endevor באמצעות ממשק Visual Studio Code. מדריך זה מתמקד בהרחבת התמיכה בשפת COBOL. הרחבת Zowe Explorer כלולה גם היא בחבילת Code4z.
...
ההרחבה Code4z מזהה קבצים עם הסיומות .COB ו-.CBL כקבצי COBOL. זה חל הן על קבצים מקומיים והן על קבצים המוחזקים ב-PDS ב-mainframe. תכונות התמיכה בשפת COBOL מופעלות באופן אוטומטי כאשר אתה פותח כל קובץ עם סיומת המזהה אותו כקובץ COBOL.
...
&#x200f;Copybooks הם קטעי קוד מקור המאוחסנים בdata sets נפרדים שאליהם מפנים בתוכנית. הרחבת התמיכה בשפת COBOL מאפשרת לך להוריד את כל ה-copybooks המוזכרים בתוכנית שלך מה-mainframe לתיקייה בסביבת העבודה שלך. כדי שתכונה זו תעבוד, עליך להגדיר פרופיל Zowe CLI `zosmf`. ניתן גם לאפשר תמיכה ב-copybooks המאוחסנים באופן מקומי בתיקיות בסביבת העבודה שלך. זה שימושי בעת עבודה עם פרויקט COBOL המאוחסן בריפו של Github.
...
# שורת הפקדודה- Zowe CLI ותוספי Zowe CLI
...
- **שימוש אינטראקטיבי ב-Zowe CLI**
     ...
     - **עזרה אינטראקטיבית**
     - **פרופילי Zowe**
     - **אינטראקציה עם data sets של z/OS**
...
שורת הפקודה נקראת באנגלית CLI- קיצור של Command Line Interface (ממשק שורת פקודה). זוהי תוכנית המאפשרת אינטראקציה עם המשתמש באמצעות קלט מבוסס טקסט. בימים הראשונים של המחשוב, ממשקי שורת פקודה היו האמצעי היחיד לאינטראקציה עם מערכות הפעלה. המצאת העכבר ופיתוח ממשקי משתמש גרפיים הובילו לחוויה שאנו מכירים כיום. ממשקי משתמש גרפיים (GUI) מעוצבים היטב בהחלט מניבים חוויה אינטראקטיבית משופרת. עם זאת, ממשקי CLI עדיין נמצאים בשימוש נרחב כיום והם חזקים מאוד. Windows shell ו-bash הם דוגמאות נפוצות למסופים שבהם כלי שורת פקודה מופעלים באופן אינטראקטיבי.

אם ממשקי GUI מעוצבים היטב מניבים חוויה אינטראקטיבית משופרת, מדוע שתשתמש ב-CLI? במילה אחת, אוטומציה. ניתן להשתמש בממשקי שורת פקודה באופן אינטראקטיבי המאפשר חקירה מהירה של פקודות זמינות. הם גם בדרך כלל מונחים-עצמית וחלקם אף מציעים תצוגות עזרה מודרניות על ידי הפעלת תוכן בדפדפן. אבל, הם גם ממשקים תכנותיים שבהם ניתן לכתוב בקלות רצפי פקודות ומשימות לסקריפטים.

## מהו Zowe CLI?

הכלי Zowe CLI הוא CLI בקוד פתוח עבור ה-mainframe. זהו כלי שניתן להריץ על Windows, Linux ו-Mac המציע אמצעי לאינטראקציה עם ה-mainframe מסביבה שבה זמינים כלי קוד פתוח מודרניים. פלטפורמות ענן כמו Amazon Web Services, Azure ו-Google Cloud Platform כולן מספקות ממשקי CLI הנמצאים בשימוש נרחב. Zowe CLI עוזר להפוך את האינטראקציה עם ה-mainframe דומה לאינטראקציה עם שירותי ענן אחרים.

בבסיסו, Zowe CLI מספק אינטראקציה מרחוק עם data sets ועבודות של z/OS, קבצי Unix System Services, פקודות TSO וקונסולה, ושירותי הקצאה. Zowe CLI הוא גם טכנולוגיה ניתנת להרחבה וקיימים תוספים רבים המרחיבים את טווח ההגעה שלו לתת-מערכות z/OS ותוכנות ספקים.

הכלי Zowe CLI מאפשר גישור בין מערכות מבוזרות ל-mainframe. בחר את השפה או כלי הקוד הפתוח המועדפים עליך ונצל אותם לפיתוח mainframe בסיוע Zowe CLI. רוצה לפתח אוטומציה ב-Python? רוצה לכתוב בדיקות ב-Node? רוצה להריץ פייפליינים של Jenkins עבור תהליכי CI? רוצה להשתמש בספריות בדיקה בקוד פתוח כמו Mocha או Jest של פייסבוק? רוצה למנף כלי איכות קוד כמו SonarQube? קדימה!

ממשקי CLI שימושיים לאוטומציה של משימות חוזרות. עבור יישומי COBOL ב-mainframe, Zowe CLI יכול לעזור לך להפוך את תהליכי הבנייה, הפריסה והבדיקה שלך לאוטומטיים. עיין ב[בלוג](https://medium.com/zowe/continuous-integration-for-a-mainframe-app-800657e84e96) זה למידע נוסף ולקוד לדוגמה שמאפשר זאת! Zowe CLI יכול גם לעזור לך להפוך משימות ניהוליות לאוטומטיות.
...
הכלי Zowe CLI הוא חבילת node והוא אחד מתוך למעלה מ-1.2 מיליון חבילות node הזמינות בספריית חבילות התוכנה npm. לאחר התקנת Node.js ו-npm במחשב הלקוח, ניתן להתקין את ה-CLI הליבתי פשוט על ידי הפקודה `npm install -g @zowe/cli@zowe-v3-lts`. קיימת שיטת התקנה חלופית אם לתחנת העבודה שלך אין גישה לnpm הציבורי. פרטים נוספים על התקנת Zowe CLI ותוספי Zowe CLI מסופקים בסעיף עתידי שכותרתו "התקנת Zowe CLI ותוספים".

### עזרה אינטראקטיבית

כדי להתחיל, אתה יכול פשוט לפתוח טרמינל ולכתוב את הפקודה `zowe`. זה יניב את העזרה ברמה העליונה.

![](Images/zowe/zowe-cli-help.png)

*איור 1. עזרה של Zowe CLI*

בדוגמה לעיל, מותקנות הרחבות מרובות. מבנה הפקודות הוא `zowe <group> <action> <object>` ואחריו פרמטרים ואפשרויות שונות הספציפיות לפקודה. לדוגמה, פקודה חוקית היא `zowe files list data-set "HLQ.*"`. פקודה זו תפרט data sets התואמים לתבנית "HLQ.*". ניתן להוסיף `-h` לכל פקודה כדי לקבל מידע נוסף. התייחסות תכופה לעזרה יכולה להיות קשה וגוזלת זמן, כך שאם לסביבה שלך יש גישה לדפדפן אינטרנט, פשוט הוסף `--help-web` או `--hw` לכל פקודה כדי להפעיל עזרה אינטראקטיבית באינטרנט.

![](Images/zowe/zowe-cli-web-help.png)

*איור 2. עזרה אינטרנטית של Zowe CLI*

תוכל גם לבדוק עותק של עזרת האינטרנט עבור Zowe CLI הליבתי ותוספי Zowe [כאן](https://docs.zowe.org/stable/web_help/index.html).
...
הכלי Zowe CLI מספק חבילה משמעותית של פונקציונליות אינטראקציה עם data sets של z/OS. עיין באיורים הבאים לפרטים על פעולות זמינות ופקודת רשימה לדוגמה.
...
### אינטראקציה עם עבודות z/OS
הכלי Zowe CLI מספק חבילה משמעותית של פונקציונליות אינטראקציה עם עבודות z/OS. עיין באיורים הבאים לפרטים על פעולות זמינות ופקודת הגשת עבודה לדוגמה.
...
### אוטומציה של job submits
בואו נחקור אוטומציה של job submit ואימות שקוד החזרה הוא 0. כמובן, נוכל גם לנתח את פלט ה-spool עבור הודעות ספציפיות מעניינות, אך נשמור על פשטות לעת עתה. לדוגמה זו, נמנף את Node.js לפיתוח חבילת אוטומציה חדשה. כדי להתחיל, אצור קובץ package.json כדי להקל על אחרים לנהל ולהתקין את הפרויקט. הוא יכיל את רשימת התלויות של הפרויקט שלי וכן את משימות האוטומציה שאפתח. דרך מהירה ליצור package.json היא להנפיק `npm init` ולענות על ההנחיות. לאחר היצירה אוסיף משימת submitJob. תוכל להוסיף כאן כל אוטומציה שתרצה. ה-package.json הסופי שלי מוצג באיור הבא. תוכל ללמוד עוד על קבצי package.json [כאן](https://docs.npmjs.com/creating-a-package-json-file).
...
כעת, במקום לשדר פקודה זו ולסקור את התוצאה שלה כדי לראות אם קוד החזרה קטן או שווה ל-4, אנו רוצים להפוך אותה לאוטומטית. ראה את היישום בסקריפט node להלן.
...
הייתי צריך להשקיע כדי לכתוב אוטומציה זו, אך עבור הגשות עבודה עתידיות אוכל פשוט להנפיק `npm run submitJob`. סביבות פיתוח משולבות כמו VS Code יכולות ליצור ממשק גרפי עבור משימות אלה ולהפוך את המשימות החוזרות על עצמן לקלות כמו לחיצת כפתור :). עבודה זו יכולה לקמפל, לקשר ו/או להריץ תוכנית COBOL.

![](Images/automating-tasks/npm-script-button-click-and-run.png)

*איור 11. כפתור של סקריפט npm והרצה לדוגמה*
...
לאחר מכן תוכל לשדרג תהליך זה על ידי פייפליין CI/CD. מהו פייפליין CI/CD? זוהי דרך אוטומטית לבנות, לבדוק ולפרוס את היישום שלך ותוכל לעשות את אותו הדבר עם פיתוח ה-COBOL שלך. האיור שלהלן מציג את הפייפליין עבור אותן משימות אוטומטיות שעשינו קודם לכן.

![](Images/automating-tasks/CircleCI-pipeline.png)

*איור 13. פייפליין CI/CD של תהליך בניית ה-COBOL "בלחיצה אחת"*
...
**הערה**: הקפד לבחור את קובץ ההתקנה הנכון עבור מערכת ההפעלה של המחשב שלך שלך, כפי שמוצג באיור 1.

## התקנת תוספים של VSCode
...
### התוסף IBM Z Open Editor

התוסף IBM Z Open Editor מביא תמיכה בשפות COBOL ו-PL/I ל-Microsoft VSCode. זוהי אחת מכמה יכולות מהדור הבא לחוויית פיתוח פתוחה עבור z/OS®. היא פועלת גם בשיתוף עם תוסף Zowe Explorer. למידע נוסף על IBM Z Open Editor, בקר בכתובת:
...
### חבילת ההרחבות Code4z

פרוייקט Code4z הוא חבילת הרחבות mainframe בקוד פתוח, הכל-כלול, למפתחים העובדים עם יישומי z/OS, המתאימה לכל רמות הניסיון ב-mainframe, אפילו למתחילים. מפתחי יישומי mainframe יכולים להשתמש בחבילת Code4z לחוויה מודרנית, מוכרת וחלקה, המסייעת להתגבר על הסתייגויות או חששות של מפתחים מסוימים לגבי חווית המשתמש המסורתית של ה-mainframe. למידע נוסף על Code4z, בקר בכתובת [https://github.com/BroadcomMFD/code4z](https://github.com/BroadcomMFD/code4z).
